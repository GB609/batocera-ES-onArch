...
const { envOrVarAbsPath } = require('utils/path');

let p = {};
p.FS_ROOT = envOrVarAbsPath('FS_ROOT', '');
p.CONFIG_ROOT = envOrVarAbsPath('CONFIG_ROOT', p.FS_ROOT + '/etc/batocera-emulationstation');
p.DROPIN_DIR = envOrVarAbsPath('DROPIN_DIR', p.CONFIG_ROOT + '/conf.d');
p.BTC_PKG_DIR = envOrVarAbsPath('BTC_PKG_DIR', p.FS_ROOT + '/opt/batocera-emulationstation');
p.BTC_BIN_DIR = envOrVarAbsPath('BTC_BIN_DIR', p.BTC_PKG_DIR + '/bin');
// specific to btc-config. Required when doing a property import/merge with the original stuff from batocera
p.INBUILD_CONFIG_PATH = envOrVarAbsPath('INBUILD_CONFIG_PATH', p.BTC_PKG_DIR + '/conf.d');
