 ...
 *
 * @see [JS adapter](./js-to-shdoc.sh.md)
...
/**
 * @section Generic helper utilities
 ...
 */
function cleanSubPathName(path) {
 ...
 * @param {string} root directory relative to the repository root to search for files
 * @param {string[]} extensions list of file extensions (without dot) to include
 * @param {string} [fileType=null] when given, will be tested against the output of `file` for executables   
 */
...
function runShDoc(sourceFile, targetPath, prefixLines, adapter = null, rootDir = DOC_ROOT) {
  ...
  let shDocResult = exec(fullCommand, options({ env: versionRoot }, UTF8)).trim().split(NL);
...
/**
 * @section Index file update helpers
