...
//const { ROMS_DIR_TAG } = require('utils/path');
const { mergeObjects, deepImplode, HierarchicKey, tokenize } = require('utils/data');
const { parseDict, FORMATS: PARSER_FORMATS, XML, PropValue } = require('io/parsers');
const writer = require('io/writers');

require('batocera-paths');

function generateGlobalConfig(
  options,
  cfgRoot = PATHS.CONFIG_ROOT, btcSysDir = PATHS.BTC_BIN_DIR, dropinPath = PATHS.DROPIN_DIR) {
  io.userOnly('Generating (etc and/or application internal) config files for batocera-emulationstation', {
    inbuildConfigDir: PATHS.INBUILD_CONFIG_PATH,
  ...
  if (isValidPath(btcSysDir)) {
    ...
    let systems = mergeDropinsToInbuild(PATHS.INBUILD_CONFIG_PATH + "/es_systems.yml", dropinTarget, modTime(targetFile));
    if (systems == null) { skipFileMessage(targetFile, dropinTarget) }
    ...
    let features = mergeDropinsToInbuild(PATHS.INBUILD_CONFIG_PATH + "/es_features.yml", dropinTarget, modTime(targetFile));
    if (features == null) { skipFileMessage(targetFile, dropinTarget) }
  ...
  if (isValidPath(cfgRoot)) {
    ...
    let properties = mergeDropinsToInbuild([
      PATHS.INBUILD_CONFIG_PATH + "/batocera.conf",
      PATHS.INBUILD_CONFIG_PATH + "/configgen-defaults.yml",
      PATHS.INBUILD_CONFIG_PATH + "/configgen-defaults-x86_64.yml"
    ], [
      summaryFilesDir + "/supported_systems.json",
      dropinTarget
    ], Math.min(...targetFile.map(modTime)));
    if (properties == null) { skipFileMessage(targetFile, dropinTarget) }
...
function skipFileMessage(targetFile, dropinDir) {
  io.warn(`Skip re-creation of ${targetFile} because there were no changes in ${dropinDir}`)
}
...
function buildComment(mergeResult, options, dropinDir) {
  ...
  let sourceList = `\nsources:\n${mergeResult.sourceFiles.map(_ => '/' + relative(PATHS.FS_ROOT, _)).join('\n')}`;
...
function mergeDropinsToInbuild(base, dropinDir, lastMergedDate = 0) {
  ...
  let dropinFiles = dropinDir
    ...
    .filter(_ => PARSER_FORMATS.includes(extname(_).substring(1)));
...
function generateBtcConfigFiles(properties, targetDir = PATHS.CONFIG_ROOT, writerOptions = {}) {
  ...
  let commentLines = writerOptions.comment || '';
  for (let [filename, props] of Object.entries(byTargetFile)) {
    ...
    if (numLines > 0) {
      ...
      writer.conf.write(props, finalFilePath, { comment: commentLines, verbose: writerOptions.verbose });
    }
  }
}
...
class ControllerConfig {
  static readControllerSDL(customFiles, controllerIds = null) {
    const internalConfig = `${PATHS.BTC_BIN_DIR}/es_input.cfg`;
