...
const VALIDATORS = new Proxy({
  ...
  varArgs: function(pickArgumentCondition, unused, ...argArray) {
    ...
    for (let i = 0; i < argArray.length; i++) {
      let checkResult = pickArgumentCondition.call(this, argArray[i]);
      if (checkResult instanceof ValidatorResult) {
        if (checkResult.success) { resultArray.push(checkResult.value); continue; }
        else { break }
      }
      if (!isValidResult(checkResult)) { break }
      else { resultArray.push(argArray[i]) }
    }
    return ValidatorResult.of(true, resultArray.length, resultArray);
  },
...
function _pseudoBind(valName, validator, ...rest) {
  ...
  switch (rest.length) {
    ...
    case 0:
      ...
      break;
    default:
      valArgs.push(rest[0], rest.slice(1));
      break;
  }
  let valWrapper = {
